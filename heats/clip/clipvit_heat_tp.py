...
text = "A cat is sitting on the ground."
with torch.no_grad():
    text_tokens = clip.tokenize([text]).to(device)
    t_feat = model.encode_text(text_tokens)
...
with torch.no_grad():
    ...
    for idx, x in enumerate(mid_x):
        ...
        similarity = torch.einsum("ab,apb->ap", t_feat, p_feat).squeeze(0).cpu().numpy()
        ...
        plt.tight_layout()
        plt.savefig(f"./clip/clipvit_heat_tp/cat_block_{idx}.jpg", dpi=300)
        plt.close()
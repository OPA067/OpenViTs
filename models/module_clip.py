...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        x_list = [x.permute(1, 0, 2) for x in x_list]
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, mask=None):
        hidden_list = self.visual(image.type(self.dtype))
        hidden_list = [self.visual.ln_post(x) @ self.visual.proj for x in hidden_list]

        x_list = [h[:, 0, :] for h in hidden_list]
        if return_hidden:
            return x_list, hidden_list
